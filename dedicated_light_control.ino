...
void loop(){
  if (Serial.available()){// Прием данных из буфера 
    ...
    if (val == '1'){  // Если пришла единица, то включаем свет командо digitalWrite со вторым параметром HIGH
      digitalWrite(Reley_1, HIGH);
      Serial.println("Reley — ON");//Вывод сообщения в bluetooth терминале о том, что свет включен 
    }
    if ( val == '0'){// Если пришел ноль, то выключаем свет той же командой только со вторым аргументом LOW
      digitalWrite(Reley_1, LOW);
      Serial.println("Reley — OFF");// Вывод сообщения в bluetooth терминале о том, что свет выключен
    }
  }
}
